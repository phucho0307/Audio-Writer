...
export default function StoryPage({
  ...
  async function setPricing(patch: {
    freeChapters?: number;
    unlockPrice?: number;
  }) {
  ...
  const frozen = story.visibility !== "PRIVATE" && branchId === liveBranchId;
  ...
  const aiSpent =
    quota !== null && !quota.unlimited && quota.totalRemaining === 0;

  return (
    <div className="mx-auto flex max-w-6xl gap-10 px-6 py-10">
      {/* Sticky rail. Eight chapters is a long scroll, and the list is the
          fastest way back to the one you were working on - keeping it in view
          is the whole reason it moved out of the flow. */}
      <aside className="sticky top-6 hidden h-[calc(100vh-6rem)] w-56 shrink-0 overflow-y-auto lg:block">
        <Contents
          rail
          chapters={shown}
          branchId={branchId}
          editable={chaptersEditable}
          editingId={editingId}
          lockedCount={read.access.lockedCount}
          onPick={setEditingId}
        />
      </aside>

      <main className="flex min-w-0 max-w-3xl flex-1 flex-col gap-8">
        {/* ---- header ---- */}
          ...
          <section className="flex flex-col gap-3 rounded-lg border border-[var(--color-accent)] bg-[var(--color-accent-soft)] p-4">
            <div>
              ...
              <p className="text-[12px] text-[var(--color-muted)]">
                {forkAt + 1} chương đầu được giữ nguyên từ bản gốc. Bạn viết
                tiếp từ chương {forkAt + 2}. Nhánh chỉ được tạo khi bạn lưu —
                rời trang bây giờ thì không có gì được lưu lại.
              </p>
            </div>
        ...
        <section className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {empty ? (
              <button
                onClick={() => runStream(`/ai/stories/${id}/seed`, "seed")}
